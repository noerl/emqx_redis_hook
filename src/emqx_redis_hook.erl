...
register_metrics() ->
    [emqx_metrics:new(MetricName) || MetricName <- ['redis_hook.client_connected',
                                                    'redis_hook.client_disconnected',
                                                    'redis_hook.client_subscribe',
                                                    'redis_hook.client_unsubscribe',
                                                    'redis_hook.session_created',
                                                    'redis_hook.session_subscribed',
                                                    'redis_hook.session_unsubscribed',
                                                    'redis_hook.session_terminated',
                                                    'redis_hook.message_publish',
                                                    'redis_hook.message_deliver',
                                                    'redis_hook.message_acked']].
...
on_client_connected(#{client_id := ClientId, username := Username}, 0, ConnInfo, _Env) ->
    emqx_metrics:inc('redis_hook.client_connected'),
...
on_client_disconnected(#{client_id := ClientId, username := Username}, Reason, _Env)
    when is_atom(Reason) ->
    emqx_metrics:inc('redis_hook.client_disconnected'),
...
on_client_subscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('redis_hook.client_subscribe'),
...
on_client_unsubscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('redis_hook.client_unsubscribe'),
...
on_session_created(#{client_id := ClientId}, SessInfo, _Env) ->
    emqx_metrics:inc('redis_hook.session_created'),
...
on_session_subscribed(#{client_id := ClientId}, Topic, Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('redis_hook.session_subscribed'),
...
on_session_unsubscribed(#{client_id := ClientId}, Topic, _Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('redis_hook.session_unsubscribed'),
...
on_session_terminated(#{client_id := ClientId}, Reason, _Env) when is_atom(Reason) ->
    emqx_metrics:inc('redis_hook.session_terminated'),
...
on_message_publish(Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    ...
    with_filter(
      fun() ->
        emqx_metrics:inc('redis_hook.message_publish'),
...
on_message_deliver(#{client_id := ClientId, username := Username}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
  with_filter(
    fun() ->
      emqx_metrics:inc('redis_hook.message_deliver'),
      {FromClientId, FromUsername} = format_from(Message),
...
on_message_acked(#{client_id := ClientId}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('redis_hook.message_acked'),
        {FromClientId, FromUsername} = format_from(Message),
